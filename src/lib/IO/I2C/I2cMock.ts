...
export type BufferCallback = (error: any, bytesReadOrWritten: number, buffer: Buffer) => any;

export type ResultCallback<T> = (error: any, result: T) => any;
...
export const mock = {
  ...
  i2cRead(address: number, length: number, buffer: Buffer, callback: BufferCallback): void {
    buffer.fill(1, 0, length - 1);
    callback(null, length, buffer);
  },
  i2cReadSync(address: number, length: number, buffer: Buffer): number {
    buffer.fill(0x01, 0, length - 1);
    return length;
  },
  i2cWrite(address: number, length: number, buffer: Buffer, callback: BufferCallback): void {
    callback(null, length, buffer);
 ...
 *     A new I2cBus object.
 */
export function openSync(busNumber: number): I2cBus | null {
  return mock;
}
