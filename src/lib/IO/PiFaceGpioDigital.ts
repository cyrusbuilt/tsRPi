...
export default class PiFaceGpioDigital extends PiFaceGpioBase {
     ...
     * @constructor
     */
    constructor(pin: PiFacePins | null, initialValue: PinState = PinState.LOW, spiAddress?: number, spiSpeed?: number, test?: boolean) {
    ...
    public poll() {
        ...
        this.shuttingDown = false;
        this.pollTimer = setInterval(() => { this.backgroundPoll() }, 50);
    }
    ...
    public dispose() {
        ...
        this.cancelPoll();
        this.spi.close((err) => { console.log(err) });
        super.dispose();
    }
